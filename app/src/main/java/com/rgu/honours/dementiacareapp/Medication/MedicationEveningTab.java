...
public class MedicationEveningTab extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.medication_morning_tab, container, false);
        medicationList = view.findViewById(R.id.medicationMorningView);
        ...
        medicationRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot ds : dataSnapshot.getChildren()) {
                    ...
                    if (ds.child("evening").getValue().equals(true)) {
    ...
    public class MyAdapter extends RecyclerView.Adapter<MyAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(final MyAdapter.ViewHolder holder, int position) {
            ...
            if (medication.getEveningTaken() == 1) {
                checkBox.setChecked(true);
            }
            final DatabaseReference medRef = dbRef.child("Users").child(userId).child("Patients").child(patientId).child("Medication");
            medRef.child(medication.getId()).child("eveningTakenTime").addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
            ...
            checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    if (isChecked) {
                        medRef.child(medication.getId()).child("eveningTaken").setValue(1);
                        medRef.child(medication.getId()).child("eveningTakenTime").setValue(System.currentTimeMillis());
                    }
                    if (!isChecked) {
                        medRef.child(medication.getId()).child("eveningTaken").setValue(0);
                        medRef.child(medication.getId()).child("eveningTakenTime").setValue(0);
